...
import com.dk.mp.hy.adapter.TxlAdapter;
import com.dk.mp.hy.db.RealmHelper;
...
public class TxlDepartActivity extends MyActivity implements EasyPermissions.PermissionCallbacks{
    ...
    TxlAdapter mAdapter;
    TxlAdapter bAdapter;
    List<HyJbxx> mList = new ArrayList<>();//星标同事
    List<HyDepartMent> mData = new ArrayList<>();//所有部门
    private RealmHelper mRealmHelper;
    private boolean isxb = true;

    @Override
    protected int getLayoutID() {
        return R.layout.app_hy;
    }
    ...
    public void initView(){
        ...
        BroadcastUtil.registerReceiver(this, mRefreshBroadcastReceiver, new String[]{"hy_persons"});
    }
            ...
            public void onSuccess(JSONObject result)  {
                try {
                    if (result.getInt("code") != 200) {
                        ...
                        List<HyDepartMent> departments = new Gson().fromJson(json,new TypeToken<List<HyDepartMent>>(){}.getType());
    ...
    public void getBmDataByLocal(int par){
        List<HyDepartMent> departments = mRealmHelper.queryAllDepartment();//从本地存储获取数据
     ...
     */
    public void getXb(){
        List<HyXbPersons> xbtss = mRealmHelper.queryALlXb();
        if(xbtss!= null && xbtss.size()>0){
            isxb = true;
            xbts_lin.setVisibility(View.VISIBLE);
            List<HyJbxx> jbxxs = new ArrayList<>();
