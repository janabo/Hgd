...
import com.dk.mp.hy.adapter.TxlSearchAdapter;
...
public class SearchActivity extends MyActivity{
    ...
    private List<HyJbxx> mList = new ArrayList<>();
...
//        map.put("key", mKeywords.getText().toString());
        HttpUtil.getInstance().postJsonObjectRequest("apps/yellowpage/query?key="+mKeywords.getText().toString(), map, new HttpListener<JSONObject>() {
            @Override
            public void onSuccess(JSONObject result)  {
                try {
                    if (result.getInt("code") != 200) {
                        ...
                        List<HyJbxx> departments = Manager.getPeoples(result);
